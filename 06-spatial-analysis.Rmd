...
Then, you can recombine your calculated attribute values (in `bearstats`) to your original layer (`bear`) with `spCbind()` from the `{maptools}` package [@R-maptools]:
...
*  Calculating the **home range** of an animal, i.e., finding a polygon that describes where most of the relocations occurred.
...
First, you should get a nicer basemap than a R's blank slate. Use the `{dismo}` [@R-dismo] and `{raster}` [@R-raster] packages to get one:
...
Now, plot the crossings in blue on top of the basemap with the railroad in red. Don't forget the scale bar and north arrow using the `{prettymapr}` [@R-prettymapr] package!
...
knitr::include_graphics("MakePNG/FinalMap.png")
```

Where are bears crossing the railroad? It looks like there are two areas of the railroad that get the most bear activity. 
...
5.  How far from the centroid of their homerange are the caves? (_Hint:_)
6.  Place arrows on the final map you made in Section \@ref(base-maps) pointing to the two areas with the most crossing activity. Style the arrows as you like. (_Hint: look back to Section \@ref(error-bars) for how arrows can be drawn._)
