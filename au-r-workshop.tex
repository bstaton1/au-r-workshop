...
comprehensive. R has gained lots of popularity as a statistics software
and is commonly used both in academia and governmental resource
agencies. I think this popularity is a result of its power, flexibility,
intuitiveness, and price (free!). For many students, this chapter may be
the one that is most immediately useful.

\textbf{IMPORTANT NOTE}: If you did not attend the session corresponding
to the Chapter \ref{ch1} material or Chapter \ref{ch2}, you are
recommended to walk through those chapters' contents before proceeding
to this material. This is because you will be using some of the commands
and terminology presented previously. If you are confused about a topic,
you can use \textbf{CTRL + F} to find previous cases where that topic
...
predictor variable(s) (\(x_n\)) and is often written:
...
\section{ANOVA: Categorical
predictors}\label{anova-categorical-predictors}

ANOVA models attempts to determine if the mean of different groups are
...
always see how R is ordering your factor:
...
\texttt{dat\$type\_relevel\ =\ relevel(dat\$type,\ ref\ =\ "wild")}.
...
\item
  \(\beta_1\) interpretted as the difference in mean \texttt{fecund}
...
you see that the mean fecundity of hatchery fish is about 1846 g and
that the average wild fish is about 713 g heavier than the average
...
And you can plot the fit. Study these lines to make sure you know what
each is doing. Use what you know about the meanings of the three
coefficients to decipher the two \texttt{abline} commands.
...
You can run the interactive ANCOVA model like this:
...
\section{An Example GLM: Logistic
Regression}\label{an-example-glm-logistic-regression}
...
the \(\beta\) coefficients and the \(x_ij\) data) and need to get
...
This will be your response variable and your model will predict the
...
\item
  ...
  probability of success forr fish with zero weight. Remember
  ``success'' is defined as having at least 70\% egg survival to the
  stage of interest.
\item
