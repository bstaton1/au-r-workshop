...
\item
  \textbf{Chapter \ref{ch3}} covers the basics fitting statistical
  models using built-in functionality for generalized linear models as
  well as non-linear models.\\
\item
  \textbf{Chapter \ref{ch4}} covers the basics simulation modeling in R.
\item
  \textbf{Chapter \ref{ch5}} covers the basics the \texttt{dplyr} and
...
Many of the data sets used in this book were simulated (using R!) by the
author. In cases where the data set used is not simulated, this is noted
and a citation to the data source is provided.

The data sets used in this book are hosted on a
\href{github.com}{GitHub} repository: \textbf{INSERT GITHUB INFO HERE}.
\textbf{INSERT INSTRUCTIONS HERE}

\section{Development of this book}\label{development-of-this-book}

This book was created using R and the packages \texttt{bookdown} and
\texttt{knitr}. It is hosted on \href{https://pages.github.com/}{GitHub
Pages}
...
First off, you will need to get R and RStudio\footnote{While it is
  possible to run R on it's own, it is clunky. You are strongly advised
...
The data files for this workshop are found in the \textbf{GitHub
repository, reminder}. Navigate to this repository, find the file called
\texttt{streams.csv}, download that file\footnote{To download a single
  file from GitHub, click on the file, then click ``Raw'' in the toolbar
  on the top right, then right-click anywhere in the document and click
  ``Save As\ldots{}''. You can also download a zip folder with all of
  the \texttt{.csv} files, go to ``Clone or Download''}, and place it in
your working directory for this session. This document assumes your
working directory is
\texttt{C:/Users/YOU/Documents/R-Workshop/Chapter1}, though the one you
are actually using may be slightly different (the \texttt{YOU} should at
least be changed for your computer). We can read the contents of
\texttt{streams.csv} into R using this code (ensure your working
directory is set first):
...
\textbf{If \texttt{streams.csv} is in your working directory, all you
need to provide is the file name.} If you do not get an error,
congratulations! However, if you get an error that looks like this:
...
then fear not. This has must be among the most common errors encountered
...
\item
  ...
  that you put \texttt{streams.csv} in \texttt{/R-Workshop/DataHere}.
  Here are two examples that would both read in the file in this case:
\end{enumerate}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dat =}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"C:/Users/YOU/Documents/R-Workshop/DataHere/streams.csv"}\NormalTok{)}
\NormalTok{dat =}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"../DataHere/streams"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
folder up from the working directory for a folder called
\texttt{DataHere} then find a file called \texttt{streams.csv} in it.''
...
There is one error in the data set ``ponds.csv'' (\textbf{found on
GitHub, reminder}). After you download the data and place it in the
appropriate directory, make sure you open this file and fix it
\emph{before} you bring it into R. Refer back to the information about
reading in data (Section \ref{read}) to make sure you find the error.
...
\item
  ...
  treatment (\_Hint: \texttt{table} will have only two arguments.).
\item
...
Chapter. Create a new R script called \texttt{Ch2.R} and save it in a
directory like \texttt{C:/Users/YOU/Documents/R-Workshop/Chapter2}. Set
your working directory to that location. Revisit Sections \ref{scripts}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-72-1} \end{center}
...
the factor. Download the \texttt{creel.csv} data set from the
\textbf{GitHub repository, reminder}, place it in the same directory as
your R script \texttt{Ch2.R} (which is the one you should be working on
right now).
...
\item
  ...
  set from \textbf{GitHub, reminder} and read it into R. Produce a basic
  summary of the data and take note of the data classes, missing values
  (\texttt{NA}), and the relative ranges for each variable.
\item
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-107-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-115-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-127-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-135-1} \end{center}
...
\protect\hyperlink{data-sets}{Datasets}
