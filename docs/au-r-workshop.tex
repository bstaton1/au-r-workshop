...
and was last built on 10-10-2018.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{dat[}\OperatorTok{!}\NormalTok{dat}\OperatorTok{$}\NormalTok{state }\OperatorTok{==}\StringTok{ "Alabama"}\NormalTok{,]}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##        state stream_width   flow
## 6    Georgia        39.42  57.63
## 7    Georgia        79.04 123.78
## 8    Georgia        17.65  31.90
## 9    Georgia        73.02  95.64
## 10   Georgia        65.85  95.82
## 11 Tennessee        54.83  82.30
## 12 Tennessee        57.38     NA
## 13 Tennessee        75.93 120.06
## 14 Tennessee        94.65 135.63
## 15 Tennessee        73.66 120.61
## 16   Florida        30.62  28.75
## 17   Florida        20.43  46.88
## 18   Florida        80.24 111.34
## 19   Florida        94.15 149.54
## 20   Florida        92.50 131.22
\end{verbatim}
...
(regions fill from left to right, and from top to bottom). Make 3
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# 1) extract and sort the hours for two fisheries from fewest to most hours fished}
...
code above: changing the margins using \texttt{par(mar)} (the margins
are always in this order:
\texttt{c("bottom",\ "left",\ "top",\ "right")}) and the rotation of
y-axis tick mark labels using \texttt{las\ =\ 1}.

\section{Exporting Plots}\label{exporting-plots}

There are two main ways to save plots. First is a quick-and-dirty
point-and-click method that saves the plots in low-resolution. Because
you have to point and click, this cannot be automated. The second method
produces cleaner-looking high-resolution plots with code that can be
embedded in your script, ensuring the same exact plot will be created
...
PDFs, tiff files, jpegs, etc. You should save one of the plots you made
in this Chapter using this approach.
...
southernmost population of all sockeye populations in the world
...
combination of categorical and continuous (ANCOVA). The main purpose of
fitting a linear model is to estimate the coefficients (\(\beta\)), and
in some cases to determine if their values are ``significantly''
...
predictor variable, look for those variables in a data frame called
\texttt{dat}''.
...
each line is doing. Use what you know about the meanings of the three
...
where \(lp_i = logit(p_i)\). Make an R function to perform the inverse
...
Because all of the fitted beta coefficients are on the log scale, you
cannot make easy interpretations about the relationships between the
...
eggs laid that make it to the ``eyed-egg'' stage. Any year with over
70\% egg survival we will consider a successful year, so create a new
...
\item
  ...
  \item
    ...
    weight, success became * \(e^{\beta_1}\) times as likely to
    happen''.
  \end{itemize}
\item
...
\begin{verbatim}
##  [1] "live" "live" "live" "live" "live" "die"  "live" "live" "live" "live"
\end{verbatim}
...
result in significantly less code (see Section \ref{vector-math}).
...
\item
  Function writing can lead to more readable code (if people know what
  your functions do)
\end{itemize}
...
related to the probability of making a type two error: failing to reject
a false null hypothesis. In other words, having high power means that
you have a high chance of detecting an effect if an effect truly exists.
Normally, power depends on the effect size, and the sample size
\texttt{n}. Strong effects are easier to detect than weak ones, and more
samples increase the test's sensitivity (the ability to detect weak
effects).
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-186-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-188-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-190-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-198-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-200-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-205-1} \end{center}
...
\item
  ...
  with the argument \texttt{interval\ =\ "confidence"}.
\end{enumerate}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-225-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-231-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-235-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-252-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-255-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-270-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-272-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-276-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bearstats =}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{stat =}\NormalTok{ bearstats}\OperatorTok{$}\NormalTok{NAME_}\DecValTok{1}\NormalTok{)}
\KeywordTok{table}\NormalTok{(bearstats}\OperatorTok{$}\NormalTok{stat) }
\end{Highlighting}
\end{Shaded}
...
\NormalTok{bear =}\StringTok{ }\KeywordTok{spCbind}\NormalTok{(bear, bearstats}\OperatorTok{$}\NormalTok{stat)}
\KeywordTok{head}\NormalTok{(bear}\OperatorTok{@}\NormalTok{data)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{table}\NormalTok{(bear}\OperatorTok{$}\NormalTok{ID, bear}\OperatorTok{$}\NormalTok{bearstats.stat)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-282-1} \end{center}
...
The \texttt{\{leaflet\}} package \citep{R-leaflet} allows you to make
interactive maps. This will only work if your output is
HTML-based\footnote{If you're viewing this in PDF format, go to:
...
\addcontentsline{toc}{section}{Exercise 6 Bonus}
...
\addcontentsline{toc}{section}{Exercise 1 Solutions}

\hypertarget{ex1a-answers}{\subsection*{Exercise 1A
Solutions}\label{ex1a-answers}}
\addcontentsline{toc}{subsection}{Exercise 1A Solutions}
...
\addcontentsline{toc}{subsection}{Exercise 1B Solutions}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-335-1} \end{center}

\section*{Exercise 4 Solutions}\label{ex4-answers}
\addcontentsline{toc}{section}{Exercise 4 Solutions}

\hypertarget{ex4a-answers}{\subsection*{Exercise 4A
Solutions}\label{ex4a-answers}}
\addcontentsline{toc}{subsection}{Exercise 4A Solutions}
...
\addcontentsline{toc}{subsection}{Exercise 4B Solutions}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-345-1} \end{center}

\hypertarget{ex4c-answers}{\subsection*{Exercise 4C
Solutions}\label{ex4c-answers}}
\addcontentsline{toc}{subsection}{Exercise 4C Solutions}
...
\addcontentsline{toc}{subsection}{Exercise 4D Solutions}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-356-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-360-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-361-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-361-2} \end{center}

\hypertarget{ex4e-answers}{\subsection*{Exercise 4E
Solutions}\label{ex4e-answers}}
\addcontentsline{toc}{subsection}{Exercise 4E Solutions}
...
\addcontentsline{toc}{subsection}{Exercise 4F Solutions}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-376-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-383-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-394-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-396-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-398-1} \end{center}
...
\begin{center}\includegraphics{au-r-workshop_files/figure-latex/unnamed-chunk-402-1} \end{center}
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(maptools)}
\NormalTok{caves =}\StringTok{ }\KeywordTok{spTransform}\NormalTok{(caves, }\KeywordTok{proj4string}\NormalTok{(stats))}
\NormalTok{cavestats =}\StringTok{ }\KeywordTok{over}\NormalTok{(caves, stats)}
\NormalTok{caves =}\StringTok{ }\KeywordTok{spCbind}\NormalTok{(caves,cavestats}\OperatorTok{$}\NormalTok{NAME_}\DecValTok{1}\NormalTok{)}
\KeywordTok{table}\NormalTok{(caves}\OperatorTok{$}\NormalTok{cavestats.NAME_}\DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
