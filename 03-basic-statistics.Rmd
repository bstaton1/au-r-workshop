...
*  $\beta_1$: the slope (change in `fecund` for one unit change in `weight`)
...
If $x_{i1}$ is `type` coded with 0's and 1's as in \@ref(anova) and $x_{i2}$ is `weight`, then the coefficients are interpretted as:
