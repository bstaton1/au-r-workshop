...
*  $\beta_1$ is interpretted as the difference in mean `fecund` between the `"wild"` level and the `"hatch"` level. 
...
*  $\beta_0$: the log odds of success for a fish with zero weight (which is not all that important). $e^{\beta_0}$ is the odds of success for fish with zero weight, and $expit(e^{\beta_0})$ is the probability of success for fish with zero weight. Remember "success" is defined as having at least 70% egg survival to the stage of interest.
...
```{r dist-table, results = "asis", echo = F}
...
knitr::kable(tab, align = "lll", caption = "A brief description of probability distributions commonly used in ecological problems, including the function suffix in R.")

```

In R, there are four different ways to use each of this distribution functions (each as a separate prefix):
...
* **The cumulative density function** (`p-`): the sum of the probability densities for all random variables below some value.
...
hist(random, breaks = 100, col = "grey", main = "rnorm()",
...
## Bonus Topic: Non-linear Regression
...
First, aquire the `feeding.csv` data set from the  [GitHub repo](https://www.github.com/bstaton1/au-r-workshop-data) and place it in your working directory. Read the data into R: 
...
You can fit this model in R using the `nls` function, behaves very similarly to the `lm` function. 
...
You can obtain similar output as from `lm` using the `summary`, `coef`, and `predict`. Draw the fitted line over top of the data:
