...
Oftentimes, you'll have a fitted model that you wish to propagate the uncertainty from to some derived quantity. Consider the case of the **von Bertalanffy growth model**. This is a non-linear model used to predict the size of an organism (weight or length) based on its age. The model can be written for a non-linear regression model (see Section \@ref(nls)) as:
...
Acquire the data set `growth.csv` (found in the [GitHub repository](https://github.com/bstaton1/au-r-workshop-data), or go [here](#data-sets) for more info). Read in and plot the data:
...
1.  Resample from the originial data (with replacement)
...
Obtain the fraction of iterations that resulted in the mean length-at-age being greater than 450 mm. This is interpretted as the probability that the average-sized fish of each age is mature:
...
Acquire the data set `ponds.csv` (found in the [GitHub repository](https://github.com/bstaton1/au-r-workshop-data), or go [here](#data-sets) for more info). This is the same data set used for [Exercise 1B](#ex1b), revisit that exercise for details on this hypothetical data set. Read in and plot the data:
...
It appears as though there is a relatively strong signal indicating a difference. Use the permutation test to determine if it is statistically significant. Step 1 from the pseudocode is to caluclate the observed difference between groups:
...
Notice the null distribution is centered on zero: that's because the assumption it makes is that there is no difference. The observation (blue line) falls way in the upper tail of the null distribution, indicating it is unlikely an effect that large was observed by random chance. The two-tailed p-value can be calculated as:

```{r}
mean(abs(Dnull) >= Dobs)
```

Very few (or zero) of the random data sets resulted in a difference greater than what was observed, indicating there is statisitcal support to the hypothesis that there is a non-zero difference between the two nutrient treatments.
...
2.  Simulate flipping the same unfair coin 100 times, but suing `sample` instead. Determine what fraction of the flips resulted in heads.
...
4.  Simulate rolling the same die 100 times, but use the function `rmultinom` instead. Look at the help file for details on how to use this function. Determin what fraction of the rolls resulted in an odd number.
5.  
6.
