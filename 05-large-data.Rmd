...
Any data analyst will tell you that oftentimes the most difficult part of an analysis is simply getting the data in the proper format. Real data sets are messy: they have missing values, variables are often stored in multiple columns that would be better stored as rows, the same factor level may be coded as two or more different types^[`"hatch"`, "`Hatch`", and `"HATCH"` are all treated differently in R], and the required data are in several separate files. You get the point: sometimes significant data-wrangling may be required before you perform an analysis.
...
Download the data sets `daily_escape.csv` and `daily_catch.csv` from the [GitHub repo](https://github.com/bstaton1/au-r-workshop-data) (or go [here](#data-sets) for details) and place them in your working directory.
...
The results tell you that there is an `r abs(round(coef(summary(fit))[2,1], 3))` day decrease in median run date for every 1 year that has gone by and that it is not significantly different from a zero day decrease per year. 
...
Download the data file `asl.csv` (**a**ge, **s**ex, **l**ength) from the [GitHub repo](https://github.com/bstaton1/au-r-workshop-data) (or go [here](#data-sets) for details) and save it in your working directory for this chapter. Create a new R script `Ex5.R` there. Open this script and read the data file into R. If this is a new session, load the `dplyr` package.
